function MAPMatlabGendocex ()
	...
	disp('>>> disp(D{4});');
	disp(D{4});
	disp('>>> m = MarginalMomentsFromMMAP(D,1);');
	...
	disp('>>> disp(H{3});');
	disp(H{3});
	disp('>>> Cm = SimilarityMatrix(H{1},D{1});');
	Cm = SimilarityMatrix(H{1},D{1});
	disp('>>> err = norm(H{1}*Cm-Cm*D{1}) + norm(H{2}*Cm-Cm*D{2}) + norm(H{3}*Cm-Cm*D{3});');
	...
	disp('>>> mm = MarginalMomentsFromMMAP(D,3);');
