%  [alpha, A] = MAPPH1QLD(D0, D1, sigma, S, transToPH, prec)
%  
...
function [alpha, A] = MAPPH1QLD (D0, D1, sigma, S, transToPH, prec)
    ...
    if BuToolsCheckInput && ~CheckMAPRepresentation(D0,D1,prec)
        error('MAPPH1QLD: The arrival process (D0,D1) is not a valid MAP representation!');
    end
    
    if BuToolsCheckInput && ~CheckPHRepresentation(sigma,S,prec)
        error('MAPPH1QLD: The service distribution (sigma,S) is not a valid PH representation!');
    end

    [alpha, A] = MAPMAP1QLD (D0, D1, S, sum(-S,2)*sigma, transToPH, prec);    
end
