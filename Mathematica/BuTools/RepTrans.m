...
TestRepTransPackage[]:=Module[{vecA,vecB,m,M,A1,T,A2,A,B,Cm,alpha,beta,err},
...
Print[B];
err = Norm[A1.B-B.A2];
Assert[err<N[10^-7], "The resulting matrix T does not satisfy A1*T = T*A2!"];
...
err = Norm[A.Cm-Cm.B];
Assert[err<N[10^-7], "The original and the transformed matrix are not similar!"];
...
err = Norm[B.Cm-Cm.M];
Assert[err<N[10^-7], "The original and the transformed matrix are not similar!"];
Assert[Min[m]>-N[10^-14], "The initial vector is still not Markovian!"];
...
Assert[Max[Abs[B.vecA-vecB]]<N[10^-14], "The resulting matrix T does not satisfy T*vecA = vecB!"];
